...
(b) estimar, via Ley de Amdahl, el limite teorico de speedup del
...
el costo de EDA/visualizaciones (que es secuencial y no forma parte de
la pregunta de HPC).

VISUALIZACIONES GENERADAS (4 graficos, guardados como PNG Y mostrados
en pantalla mediante un backend interactivo activado despues de que
termina todo el computo en paralelo):
1. grafico1_gantt_secuencial_vs_paralelo.png
    Gantt chart: cada tarea (fold x modelo) como barra horizontal,
    comparando ejecucion secuencial (1 carril) vs. paralela (N carriles).
2. grafico2_heatmap_utilizacion_nucleos.png
...
ADEMAS: el script detecta automaticamente el n_jobs OPTIMO (mayor
speedup medido), que no necesariamente coincide con el maximo de
nucleos disponibles -- si son distintos, es evidencia de sobre-
suscripcion (oversubscription) y el resumen final lo señala explicitamente.
...
import pandas as pd
import matplotlib
matplotlib.use('Agg')  # evita el conflicto de backend interactivo + multiprocessing en macOS
...
# 1. precarga de datos desde github
...
# 2. pipeline con entrenamiendo de RL
...
# 3. experimentación: ejecución vs # de núcleos
...
# ------------------------------------------------------------
# Deteccion automatica del n_jobs OPTIMO (no asumir que "mas
# nucleos = mejor"): se busca el n_jobs con MAYOR speedup medido,
# no el de mayor numero de nucleos disponibles. Si el optimo es
# menor que max_cores, es evidencia de SOBRE-SUSCRIPCION
# (oversubscription): el overhead de coordinar mas procesos que
# los que el pipeline puede aprovechar supera la ganancia de
# cómputo adicional, y el tiempo total empeora en vez de mejorar.
# ------------------------------------------------------------
idx_optimo = bench_df['speedup'].idxmax()
n_jobs_optimo = int(bench_df.loc[idx_optimo, 'n_jobs'])
speedup_optimo = bench_df.loc[idx_optimo, 'speedup']
eficiencia_optima = bench_df.loc[idx_optimo, 'eficiencia']

print(f"\n>>> n_jobs OPTIMO detectado: {n_jobs_optimo} "
    f"(speedup={speedup_optimo:.2f}x, eficiencia={eficiencia_optima*100:.1f}%)")

if n_jobs_optimo < max_cores:
    fila_max_cores = bench_df.loc[bench_df['n_jobs'] == max_cores]
    speedup_max_cores = fila_max_cores['speedup'].values[0] if len(fila_max_cores) else None
    print(f"    AVISO: usar todos los nucleos disponibles ({max_cores}) NO es lo optimo. "
        + (f"Con {max_cores} nucleos el speedup CAE a {speedup_max_cores:.2f}x "
            f"(vs. {speedup_optimo:.2f}x en el optimo) — señal de sobre-suscripcion "
            "(oversubscription): el overhead de coordinar procesos adicionales supera "
            "la ganancia de computo, y el tiempo total EMPEORA en vez de mejorar."
            if speedup_max_cores is not None and speedup_max_cores < speedup_optimo
            else "El pipeline deja de escalar linealmente mas alla de este punto."))
else:
    print(f"    El maximo de nucleos disponibles ({max_cores}) tambien es el optimo medido "
        "-- no se observo sobre-suscripcion en este rango.")
...
# ============================================================
# CAMBIO DE BACKEND: DE 'Agg' A UNO INTERACTIVO
# ============================================================
# DECISION METODOLOGICA:
# Se uso el backend 'Agg' (no interactivo) durante TODO el codigo
# anterior porque es lo que evita el conflicto conocido entre el
# backend grafico interactivo de macOS y el multiprocessing de
# joblib/GridSearchCV (el TypeError que aparecio en una corrida previa
# del pipeline principal). Ahora que YA NO queda ningun computo en
# paralelo pendiente (toda la captura de tareas termino arriba), es
# seguro cambiar a un backend interactivo para poder VER las graficas
# ademas de guardarlas como PNG. Si el backend nativo de macOS no esta
# disponible (ej. corriendo en un entorno remoto/headless), se cae de
# vuelta a 'Agg' sin interrumpir el script -- las graficas se seguiran
# guardando como PNG de cualquier forma.
backend_interactivo_activo = False
for backend_candidato in ['MacOSX', 'TkAgg', 'Qt5Agg']:
    try:
        plt.switch_backend(backend_candidato)
        backend_interactivo_activo = True
        print(f"\nBackend interactivo activado: {backend_candidato} "
            "(las graficas se mostraran ademas de guardarse)")
        break
    except Exception:
        continue

if not backend_interactivo_activo:
    print("\n[Aviso] No se encontro un backend interactivo disponible en este entorno; "
        "las graficas se guardaran como PNG pero no se mostraran en pantalla.")
...
print("=" * 60)
nota_oversubscription = ""
if n_jobs_optimo < max_cores:
    nota_oversubscription = (
        f" Cabe destacar que usar el maximo de nucleos disponibles ({max_cores}) resulto "
        f"CONTRAPRODUCENTE: el speedup cayo respecto al optimo, evidenciando sobre-suscripcion "
        f"(oversubscription) — el overhead de coordinar mas procesos que los que el pipeline "
        f"puede aprovechar realmente supera la ganancia de computo adicional."
    )

print(f"""
Con n={X_train.shape[0]} pacientes en el set de entrenamiento, el
paralelismo CPU vía n_jobs de scikit-learn produjo un speedup MAXIMO
medido de {speedup_optimo:.2f}x usando {n_jobs_optimo} nucleos (el punto optimo, no
necesariamente el mayor numero de nucleos disponibles), con una
eficiencia de {eficiencia_optima*100:.1f}% en ese punto.{nota_oversubscription}
...
# ============================================================
# MANTENER LAS 4 VENTANAS ABIERTAS
# ============================================================
# Este plt.show() final SI bloquea la ejecucion (a diferencia de los
# plt.show(block=False) anteriores) -- es intencional: sin esto, el
# script terminaria y Python cerraria las 4 ventanas de golpe antes de
# que te de tiempo de verlas. Cierra las ventanas manualmente (o
# presiona Ctrl+C en la terminal) cuando termines de revisarlas.
if backend_interactivo_activo:
    print("\nMostrando las 4 graficas en pantalla -- cierra las ventanas para terminar el script.")
    plt.show()